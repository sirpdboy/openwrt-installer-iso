        ...
        # 创建配置文件
        cat > config/openwrt << 'CONFIGEOF'
        # OpenWRT安装器配置
        PACKAGES="openrc openssh chrony hdparm e2fsprogs sfdisk parted wget curl gzip lsblk util-linux coreutils"
        ...
        if [ -n "$ISO_FILE" ] && [ -f "$ISO_FILE" ]; then
            ...
            if command -v xorriso >/dev/null 2>&1; then
                xorriso -indev "$FINAL_ISO" -toc 2>/dev/null | grep -E "(Bootable|Mbr|El-Torito|UEFI)"
            fi
